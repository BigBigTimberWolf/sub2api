...
const filteredChannels = computed(() => {
  ...
  return channels.value.filter((ch) => {
    ...
    const groupHit = ch.groups.some((g) => g.name.toLowerCase().includes(q))
    const modelHit = ch.supported_models.some((m) => m.name.toLowerCase().includes(q))
    return nameHit || descHit || groupHit || modelHit
  })
})
