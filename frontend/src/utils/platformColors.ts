...
export type Platform = 'anthropic' | 'openai' | 'nvidia' | 'antigravity' | 'gemini'
...
const ACCENT_BAR: Record<Platform, string> = {
  ...
  nvidia: 'bg-gradient-to-r from-lime-400 to-lime-500',
...
const DISCOUNT: Record<Platform, string> = {
  ...
  nvidia: 'bg-lime-100 text-lime-700 dark:bg-lime-900/40 dark:text-lime-300',
...
const GRADIENT: Record<Platform, string> = {
  ...
  nvidia: 'from-lime-500 to-lime-600',
...
function isPlatform(p: string): p is Platform {
  return p === 'anthropic' || p === 'openai' || p === 'nvidia' || p === 'antigravity' || p === 'gemini'
}
