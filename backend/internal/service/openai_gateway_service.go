...
func normalizeOpenAISelectionPlatform(platform string) string {
...
func isOpenAISelectionAccountPlatformCompatible(account *Account, platform string) bool {
	...
	switch normalizeOpenAISelectionPlatform(platform) {
	case PlatformOpenAI:
		return account.IsOpenAICompatible()
	case PlatformNvidia:
		return account.Platform == PlatformNvidia
	default:
		return account.Platform == normalizeOpenAISelectionPlatform(platform)
	}
}

func (s *OpenAIGatewayService) resolveOpenAISelectionPlatform(ctx context.Context, groupID *int64) string {
	if ctx != nil {
		if forcedPlatform, ok := ctx.Value(ctxkey.ForcePlatform).(string); ok && strings.TrimSpace(forcedPlatform) != "" {
			return normalizeOpenAISelectionPlatform(forcedPlatform)
		}
	}
...
func (s *OpenAIGatewayService) resolveOpenAIRequestPlatform(ctx context.Context, groupID *int64, requestedModel string) string {
	if forcedPlatform := resolveOpenAICompatForcedPlatform(ctx); forcedPlatform != "" {
	...
	if err == nil {
		if inferred := inferOpenAICompatPlatformFromAccounts(accounts, requestedModel); inferred != "" {
			return inferred
		}
		return platform
	}

	if inferred := openAICompatKnownPlatformForModel(requestedModel); inferred != "" {
		return inferred
	}
	return platform
}
...
func isOpenAIAccountEligibleForRequest(ctx context.Context, account *Account, requestedModel string, platform string, requireCompact bool) bool {
	if account == nil || !account.IsSchedulable() || !account.IsOpenAICompatible() {
		return false
	}
	if requestedModel != "" && !isOpenAICompatAccountModelSupportedForSelection(ctx, account, requestedModel, platform) {
		return false
	}
...
func (s *OpenAIGatewayService) tryStickySessionHit(ctx context.Context, groupID *int64, sessionHash, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, platform string) *Account {
	...
	if !isOpenAISelectionAccountPlatformCompatible(account, platform) {
	...
	if !isOpenAIAccountEligibleForRequest(ctx, account, requestedModel, platform, false) {
		return nil
	}
...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *int64, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, platform string) (*Account, bool) {
	...
	for i := range accounts {
		...
		if !isOpenAISelectionAccountPlatformCompatible(acc, platform) {
			continue
		}
...
func (s *OpenAIGatewayService) selectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool) (*AccountSelectionResult, error) {
	...
	if sessionHash != "" {
		...
		if accountID > 0 && !isExcluded(accountID) {
			...
			if err == nil {
				...
				if !clearSticky && isOpenAIAccountEligibleForRequest(ctx, account, requestedModel, platform, false) {
					account = s.recheckSelectedOpenAIAccountFromDB(ctx, account, requestedModel, requireCompact, platform)
					if account == nil {
						_ = s.deleteStickySessionAccountID(ctx, groupID, sessionHash)
	...
	for i := range accounts {
		...
		if requestedModel != "" && !isOpenAICompatAccountModelSupportedForSelection(ctx, acc, requestedModel, platform) {
			continue
		}
...
func (s *OpenAIGatewayService) resolveFreshSchedulableOpenAIAccount(ctx context.Context, account *Account, requestedModel string, requireCompact bool, platform string) *Account {
	...
	if !isOpenAISelectionAccountPlatformCompatible(fresh, platform) || !isOpenAIAccountEligibleForRequest(ctx, fresh, requestedModel, platform, requireCompact) {
		return nil
	}
	return fresh
}
...
func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDB(ctx context.Context, account *Account, requestedModel string, requireCompact bool, platform string) *Account {
	...
	if s.schedulerSnapshot == nil || s.accountRepo == nil {
		if !isOpenAISelectionAccountPlatformCompatible(account, platform) || !isOpenAIAccountEligibleForRequest(ctx, account, requestedModel, platform, requireCompact) {
			return nil
		}
		return account
	}
	...
	if !isOpenAISelectionAccountPlatformCompatible(latest, platform) || !isOpenAIAccountEligibleForRequest(ctx, latest, requestedModel, platform, requireCompact) {
		return nil
	}
	return latest
}
